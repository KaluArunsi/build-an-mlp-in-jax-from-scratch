...
def log_softmax_logits(logits):
    ...
    log_sum_exp = m + jnp.log(jnp.sum(jnp.exp(shifted_logits), axis=-1, keepdims=True))
...
def compute_param_grads(params, x, one_hot_targets):
    ...
    param_grads = grad_fn(params, x, one_hot_targets)
...
def training_step(params, x, one_hot_targets, learning_rate):
    ...
    sgd_params = sgd_update_params(params, grads, learning_rate)

    return sgd_params, loss
